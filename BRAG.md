...
The build pipeline doesn't use a single agent that writes and reviews its own work. It dispatches a build agent (prover) and a separate post-gate agent (verifier) at a *lower model tier* — fable builds get sonnet reviews — floored at sonnet, because a round-2 model-tier benchmark caught haiku missing every planted defect on a review task. The asymmetry is intentional: verification is computationally cheaper than generation, and a weaker model catching issues in a stronger model's output is a stronger signal than self-review. The one deliberate inversion: security-sensitive phases run their 3-sample review on fable regardless of the build tier — for security, verification rigor beats cost asymmetry.

Gate policy adapts per phase, and what adapts is *when* the review lands, not whether it happens. Full-gate and security-sensitive phases block their commit on a passing review. Standard and Minimal phases commit on a green suite and accumulate into an un-reviewed set, which a batch review clears every N phases (N configurable per plan, default 3), before any Full phase, and once before final verification. Deferring buys something a stricter pipeline can't have: a reviewer holding several phases at once finds the incoherence in the seams between them, which per-phase reviews are structurally blind to. Independent phases with disjoint file scopes build in parallel waves, each in its own worktree, integrated by cherry-pick in plan order.
