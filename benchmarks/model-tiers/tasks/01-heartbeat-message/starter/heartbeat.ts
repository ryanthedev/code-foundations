 ...
 * re-emitted line says, given the last hashing snapshot.
 */
 ...
 *
 * BUG: always reports "still hashing…", even once hashing has actually
 * finished (`completed === total`) and the client has moved on to preparing
 * the upload. A slow manifest/upload round-trip then makes the heartbeat lie
 * about what's happening — it re-emits a hashing percentage that stopped
 * changing minutes ago.
 */
