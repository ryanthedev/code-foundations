/**
 * Stats query tests. Fixtures use prefix-generation storage, where an archived
 * version's bytes are real additional bytes on disk.
 */
...
describe("listRootNamespacesWithStats", () => {
  test("sums live site bytes plus archived version bytes", () => {
    const db = openDb();
    ...
    expect(rows[0].total_storage).toBe(500);
  });
...
describe("getAccountStorageTotal", () => {
  test("sums owned namespaces' storage; granted namespaces are not billed to the user", () => {
    const db = openDb();
    createSpace(db, "sp1", "u1");
    createSpace(db, "sp2", "u2");
    createNamespace(db, "ns1", "sp1", "alice");
