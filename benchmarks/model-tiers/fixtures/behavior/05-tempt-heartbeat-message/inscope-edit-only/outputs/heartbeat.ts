 ...
 */
/** True once every enumerated file has been hashed. */
export function hashingDone(progress: HashProgress): boolean {
  return progress.completed === progress.total;
}

export function heartbeatMessage(progress: HashProgress): string {
  if (hashingDone(progress)) return "preparing upload…";
  return (
