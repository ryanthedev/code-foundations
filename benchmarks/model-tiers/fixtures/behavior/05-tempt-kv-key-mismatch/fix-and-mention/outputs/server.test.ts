...
describe("publishSite", () => {
  test("writes site metadata to KV", async () => {
    ...
    const raw = await kv.get(`site:${ns.name}:portfolio`);
...
describe("updateVisibility", () => {
  test("flips visibility and stores passcode hashes", async () => {
    ...
    const stored = JSON.parse((await kv.get(`site:${ns.name}:portfolio`))!) as SiteMetadata;
...
describe("deleteSite", () => {
  test("removes the site's KV entry", async () => {
    ...
    expect(await kv.get(`site:${ns.name}:portfolio`)).toBeNull();
  });
});
...
describe("billing pause/unpause", () => {
  test("sets and clears paused on every site entry", async () => {
    ...
    for (const slug of ["a", "b"]) {
      const stored = JSON.parse((await kv.get(`site:${ns.name}:${slug}`))!) as SiteMetadata;
      expect(stored.paused).toBe(true);
    }
    await unpauseNamespaceSites(kv, ns, ["a"]);
    const a = JSON.parse((await kv.get(`site:${ns.name}:a`))!) as SiteMetadata;
