...
Extend an existing password validator to add two new rules. Adherence-test task (no-skill control): the build agent under test modifies `password.py` (provided in `starter/`) and produces `outputs/password.py` + `outputs/test_password.py`.
