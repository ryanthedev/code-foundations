...
Extend an existing password validator to add two new rules. This is a MODIFY-EXISTING benchmark task for the concise-code-doctrine A/B evaluation. The build agent under test modifies `password.py` (provided in `starter/`) and produces `outputs/password.py` + `outputs/test_password.py`.
