...
**Security is mandatory, not optional, whenever caller-supplied or external input reaches a sink** — a SQL/query string, a filesystem path, a shell/`exec`/`eval` call, a deserializer, or an HTML/template. For each such sink you MUST write and run a standard attack probe appropriate to it (SQL injection `' OR '1'='1`; path traversal `../`; command injection `; touch x` / `$(...)`; unsafe-deserialization payload). If the probe demonstrates the exploit — leaks data, escapes the boundary, or executes injected behavior — that is a demonstrated correctness defect → **FAIL**. This holds even though no Done-When item named it: a demonstrated exploit is a defect, never a style preference. Do not mark Security N/A while an untrusted value reaches a sink unprobed.
...
- missing defensive code no requirement asked for — **UNLESS** you demonstrate an actual exploit against untrusted input reaching a sink (a demonstrated security defect is a FAIL per Step 4, not a Note). The exclusion covers undemonstrated "could add validation" suggestions, never a proven exploit.

A FAIL must name an executable failure: **(a)** a DW item with no execution evidence, **(b)** a test that fails when run, **(c)** a defect demonstrated via TRACE, **(d)** a prompt-listed edge case the implementation does not handle, or **(e)** a demonstrated security exploit (Step 4 Security). Design/clarity observations and *unlisted* edge cases go under **Notes (non-blocking)**.
...
- ANY demonstrated security exploit — untrusted input reaching a sink (injection, traversal, unsafe exec/eval/deserialization) — → FAIL, even if no DW item named it
